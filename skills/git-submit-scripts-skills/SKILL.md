...
description: Commit and push only script and skill changes to GitHub main branch for repository git@github.com:Haha-li/openClaw-skills-own.git. Use when user asks to submit/push scripts or skills, says “git提交”, or when scripts/ or skills/ files are created or modified and should be synced.
---

# Git 提交（仅 scripts/ 与 skills/）

## 目标
- 只提交 `scripts/` 和 `skills/` 下的变更。
- 推送到 `origin/main`（仓库：`git@github.com:Haha-li/openClaw-skills-own.git`）。
- 当本次任务创建或修改了脚本/skill 时，优先在任务结束前提交推送。
- 用户明确说“git提交”时，立即执行一次提交推送。
...
4. 检查是否有可提交内容：
   - 若无变更，回复“scripts/skills 无变更，无需提交”。
5. 生成简洁提交信息并提交。
6. 推送：`git push origin main`。

## 约束
- 不提交 `scripts/`、`skills/` 以外内容。
